...
{
	//extract pocket
	vector <vector <vector <double> > > pocket;
	pocket.resize((int)lig.size());
	for(int i=0;i<(int)xyz.size();i++)
	{
		...
		for(int j=0;j<len;j++)
		{
			if(atom_rec[j]>0)pocket[atom_rec[j]-1].push_back(xyz[i][j]);
		}
	}
	//calc ligand_center
	vector <vector <double> > lig_center;
	lig_center.resize((int)lig.size());
	for(int i=0;i<(int)lig.size();i++)
	{
		lig_center[i].resize(3,0);
		int len=(int)lig[i].size();
		for(int j=0;j<len;j++)
		{
			for(int k=0;k<3;k++)lig_center[i][k]+=lig[i][j][k];
		}
		if(len>0)
		{
			for(int k=0;k<3;k++)lig_center[i][k]/=len;
		}
	}
	//calc pocket_center
	vector <vector <double> > pok_center;
	pok_center.resize((int)pocket.size());
	for(int i=0;i<(int)pocket.size();i++)
	{
		pok_center[i].resize(3,0);
		int len=(int)pocket[i].size();
		for(int j=0;j<len;j++)
		{
			for(int k=0;k<3;k++)pok_center[i][k]+=pocket[i][j][k];
		}
		if(len>0)
		{
			for(int k=0;k<3;k++)pok_center[i][k]/=len;
		}
	}
	//measurement
	//-> DCC
	vector <double> DCC;
	DCC.resize((int)pocket.size(),0);
	for(int i=0;i<(int)pocket.size();i++)
	{
		double dist=Distance(lig_center[i],pok_center[i]);
		DCC[i]=dist;
	}
	//-> DCA
	vector <double> DCA;
	DCA.resize((int)pocket.size(),0);
	for(int i=0;i<(int)pocket.size();i++)
	{
		...
		for(int j=0;j<len;j++)
		{
			double dist=Distance(pok_center[i],lig[i][j]);
	...
	for(int i=0;i<(int)pocket.size();i++)
	{
		printf("pocket %d : number %d : DCC %lf : DCA %lf \n",
			i+1,(int)pocket[i].size(),DCC[i],DCA[i]);
	}
}
...
void XYZ_DCCnDCA(string &protein_xyz, string &ligand_xyz,double thres)
{
	//---- load protein -----//
	vector <vector <vector <double> > > xyz1;
	...
	//---- calc DCC and DCA for each pocket -----//
	Calc_DCCnDCA(xyz1,xyz2,binding_lig1,binding_rec1);
}
...
int main(int argc,char **argv)
{
	//------ XYZ_DCCnDCA -------//
	{
		if(argc<4)
		{
			fprintf(stderr,"XYZ_DCCnDCA <protein_xyz> <ligand_xyz> <thres> \n");
			fprintf(stderr,"[note]: thres is for GT pocket \n");
			exit(-1);
		}
		...
		//proc
		XYZ_DCCnDCA(protein_xyz,ligand_xyz,thres);
		//exit
		exit(0);
	}
}

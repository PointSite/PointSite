...
int Mol_Ori::check_range_string(string &in)
{
	int i,len;
	int ret_val;
	char test;
...
int Mol_Ori::parse_range(string &in,vector <string> &out_map)
{
	int i,j;
	int len;
	int totnum;
...
//------------ Uni_Process ------------//
int Mol_Ori::XYZ_Input(string &file,string &range,int form,int &moln,XYZ *mol,char *AMI,char *CLE,char *ind,PDB_Residue *pdb)
{
	int ret_val;
	int i,k;
	int totnum;
